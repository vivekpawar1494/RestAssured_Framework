...
public class Patch_Update_Order extends Baseclass {

	@Test(priority = 1)
	public void patch_Update_Single_Order() throws IOException {
		try {

			extenttest = extent.createTest("Verify that user is able to update the order")
					.assignCategory(SanitySuite + "_" + env_Simple_Book);
			node = extenttest.createNode("Report");
			
			System.out.println("TEST CASE: patch_Update_Single_Order ");
			node.log(Status.INFO, "TEST CASE: patch_Update_Single_Order");
			...
			given().log().all().header("Authorization", "Bearer " + Authentication.accessAuthToken)
					.pathParam("orderId", Orders.orderId).body(dataBody).when()
					.patch(AllPagesUrls.simple_book_API + endpoint)
			...
			System.out.println("TEST CASE: patch_Update_Single_Order pass");
			node.log(Status.INFO, "TEST CASE: patch_Update_Single_Order pass");
